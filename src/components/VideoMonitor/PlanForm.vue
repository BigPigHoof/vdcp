    ...
    <div class="form-box" v-show="show">
      ...
      <div class="form">
        ...
        <div style="text-align:center;margin-top:42px;">
          <el-button :loading="isUpdating" type="primary" size="small" @click="addMonitors">确认</el-button>
        </div>
      </div>
      <el-dialog
        title="新建子目录"
...
export default {
  props: ["show", "formData", "monitorsData"],
  data() {
    return {
      pageSize: 100,
      name: "",
  ...
  methods: {
    ...
    deletePlan() {
      ...
      deleteMonitorsMenu({ menuId: this.formData.id }).then(res => {
    ...
    modifyName() {
      updateMonitorsMenu({ id: this.formData.id, name: this.name }).then(res => {
        if (res.ret == "ok") {
    ...
    chooseOrg() {
      this.nowOrgSitesData=this.$refs.select.getCheckedNodes()[0].data.monitors;
      this.allLeftData = this.$refs.select.getCheckedNodes()[0].data.monitors;
    ...
    addMonitors() {
      ...
      for (const item of this.rightPageData) {
        for (const monitor of this.allLeftData) {
          if (item == monitor.code) {
            let { monitorName, code } = monitor;
            monitors.push({ monitorName, monitorCode: code });
          }
        }
      }

      console.log(monitors);
      addMonitors({ menuId: this.formData.id, monitors }).then(res => {
    ...
    resetForm(){
         this.nowOrgId = this.monitorsData[0].id;
        this.nowOrgSitesData=this.monitorsData[0].monitors;
        this.allLeftData = this.monitorsData[0].monitors;
        this.rightPageData=[];
        this.leftPageData = this.allLeftData.slice(0, this.pageSize);
        for (const monitor of this.formData.monitors) {
          for (const item of this.leftPageData) {
            if (item.code == monitor.code) {
              this.rightPageData.push(item.code);
            }
          }
        }
    }
